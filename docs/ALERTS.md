...
6. Require the same blackout to recur on two consecutive checks before posting (5–10 min of confirmed silence at the `*/5` cadence).
   - **Cross-detector suppression:** skip any route that already has an open (un-cleared) thin-gap silence (`openSilenceLines({source:'observed-thin'})`). Pulse scans the whole roster and only gates on expectedActive ≥ 2, so a low-frequency route that clears that gate could open a *second* standalone incident for a silence `bin/bus/thin-gaps.js` is already reporting (on a different account). Thin-gaps applies the mirror guard against open pulses. `observed-clear` is line-keyed and shared, so whichever detector posts the clear releases the suppression for both — at most one open silence incident per route.
